...
folder4 = r'/data/data_mrcv/45_DATA_HUMANS/HEAD_NECK/TEST_RETEST_PCVIPR/plots/tr/pooled'
...
v2_dl_all_pooled = []
v3_dl_all_pooled = []
v2_std_all_pooled = np.array(v2_std_all_pooled)
v3_std_all_pooled = np.array(v3_std_all_pooled)
v2_dl_all_pooled = np.array(v2_dl_all_pooled)
v3_dl_all_pooled = np.array(v3_dl_all_pooled)
            ...
            if os.path.isdir(subfolder_path) and ("flow_int" in subfolder_name):
                ...
                for sub in range(1):
                    ...
                    with h5.File(os.path.join(folder2, 'FlowE_r.h5'), 'r') as hf:
                        vx = []
                        for i in range(20):
                            key = f'ph_{i:03d}_vd_1'
                            data = np.array(hf['Data'][key]).flatten()
                            vx.append(data)
                        vx = np.concatenate(vx)
                        vy = []
                        for i in range(20):
                            key = f'ph_{i:03d}_vd_2'
                            data = np.array(hf['Data'][key]).flatten()
                            vy.append(data)
                        vy = np.concatenate(vy)
                        vz = []
                        for i in range(20):
                            key = f'ph_{i:03d}_vd_3'
                            data = np.array(hf['Data'][key]).flatten()
                            vz.append(data)
                        vz = np.concatenate(vz)
                ...
                v2_std_all_pooled = np.append(v2_std_all_pooled,v2_std_all)
                v3_std_all_pooled = np.append(v3_std_all_pooled,v3_std_all)
                v2_dl_all_pooled = np.append(v2_dl_all_pooled,v2_dl_all)
                v3_dl_all_pooled = np.append(v3_dl_all_pooled,v3_dl_all)


                if volu == number_of_volunteers:
                    img_name_std = f"Time Resolved Interleaf: Pooled Data"
                    img_name_dl = f"Time Resolved Interleaf + DL: Pooled Data"

                    CI_value = bland_altman_plot(v2_std_all_pooled, v3_std_all_pooled, folder4,img_name_std,img_name_std)
                    CI_records_std.append(CI_value)
                    CI_value = bland_altman_plot(v2_dl_all_pooled, v3_dl_all_pooled, folder4,img_name_dl,img_name_dl)
